...
- Notes:
  ...
  - `statusMetadata` exposes API-owned per-status display metadata (`label`, `tone`, `transitionHint`, `nextActionLabel`) for `SUBMITTED|IN_REVIEW|PAYMENT_SETTLED|DELIVERED` so clients avoid hardcoded enum-to-copy or guidance drift.
  ...
  - `nextActionLabel` is a deterministic next-step CTA hint intended for queue filters/cards and action affordance copy.
